...
class ChatMessagesDAO(BaseDAO):
    ...
    async def find_by_id(self, chat_id: int, session: AsyncSession) -> ChatMessagesSchema:
        query = select(self.model).filter_by(id=chat_id)
        result = await session.execute(query)
        return ChatMessagesSchema.model_validate(result.scalars().all())

    async def find_all(self, session: AsyncSession, **filter_by) -> list[ChatMessagesSchema]:
        query = select(self.model).filter_by(**filter_by)
        result = await session.execute(query)
        return [ChatMessagesSchema.model_validate(user) for user in result.scalars().all()]
