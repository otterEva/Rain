...
async def create_private_chat(chat_name : str = None, session: AsyncSession = Depends(get_session),
	...
	await ChatMembersDAO.add(chat_id = chat.id, chat_user_id = current_user.id, session = session)
